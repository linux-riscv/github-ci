...
for ltp_test in ${ltp_tests[@]}; do
    ./tuxrun/run --runtime null --device qemu-riscv64 --kernel /build/$KERNEL_PATH --tests $ltp_test --results /build/squad_json/$ltp_test.json --log-file-text /build/squad_json/$ltp_test.log --timeouts $ltp_test=240 || true
    ...
    curl --header "Authorization: token $SQUAD_TOKEN" --form tests=@/build/squad_json/$ltp_test.squad.json --form log=@/build/squad_json/$ltp_test.log https://mazarinen.tail1c623.ts.net/api/submit/riscv-linux/linux-all/6.12-rc7-fixes/qemu
done
